...
EOD;
	...
	protected function setPermissions($targetDir)
	{
		...
		@chmod($targetDir.'/protected/yiic.bat',0755);
	}
	...
	protected function addFileModificationCallbacks(&$fileList)
	{
		...
		$fileList['protected/yiic.bat.php']['callback']=array($this,'generateYiic');
	}
	...
	/**
	 * Inserts path to framework's yiic.bat.php into application's yiic.bat.php
	 *
	 * @param string $source source file path
	 * @param array $params
	 * @return string modified source file content
	...
	public function generateYiic($source,$params)
	{
		...
		$yiic=$this->getRelativePath($yiic,$this->_rootPath.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'yiic.bat.php');
		$yiic=str_replace('\\','\\\\',$yiic);
		return preg_replace('/\$yiic.bat\s*=(.*?);/',"\$yiic.bat=$yiic;",$content);
	}
